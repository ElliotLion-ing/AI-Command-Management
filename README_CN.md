...
**注意**：保留此工具是为了向后兼容。**请改用 `report_feedback`** 以获得更好的用户体验。

将生成的分析报告上传到服务器进行持久存储。**支持用户提供的自定义报告名称。**
...
{
  "success": true,
  "report_path": "/opt/acmt/Commands-Analyze-Report/analyze_zoom_speech_sdk_log/analyze_zoom_speech_sdk_log_关键超时分析_20251126_143022_v1.md",
  "report_name": "analyze_zoom_speech_sdk_log_关键超时分析_20251126_143022_v1.md",
  "report_link": "https://server.example.com/reports/...",
  "message": "报告上传成功",
  "version": 1
}
...
**功能特性**：
- 📝 **自定义命名**：用户可提供 `report_name`（可选），或使用默认格式：`{command}_报告_{timestamp}_v1.md`
- 🔄 **自动版本控制**：冲突时自动增加版本号（v1 → v2 → v3）
- 📁 **自动目录创建**：首次上传时创建命令特定的报告目录
- 💾 **原子写入**：使用临时文件 + 重命名确保数据完整性
- 🔒 **安全性**：目录遍历防护、大小限制、权限控制
- 🔗 **链接生成**：如果配置了 `report_link_base_url` 则返回 HTTP 链接
