...
/**
 * Report syncer class
 * Synchronizes report metadata to remote server BEFORE file upload
...
export class ReportSyncer {
  ...
  /**
   * Sync report metadata to remote server with retry mechanism
  ...
  async sync(
    ...
    if (!preconditionResult.passed) {
      logger.error('Report sync precondition failed', new Error(preconditionResult.error_message), {
    ...
    logger.info('Starting report sync with retry mechanism', {
      url,
      commandName: commandNameWithSuffix,
      reportName,
      owner,
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      ...
      if (attemptResult.success) {
        logger.info('Report sync succeeded', {
          attempt,
      ...
      // Log failed attempt
      logger.warn(`Report sync attempt ${attempt} failed`, {
        attempt,
        maxAttempts,
    ...
    logger.error('Report sync failed after all retries', new Error(finalError), {
  ...
  static formatSyncResultForDisplay(result: SyncResult): string {
    ...
    if (result.success) {
      ...
      lines.push('');
      lines.push('✅ 数据库同步成功，报告元数据已记录到 ZCT 数据库');
